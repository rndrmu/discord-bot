	...
	async execute(client, message, args, { config, Ticket }) {
		...
		if (!channel) {
			...
			if (!ticket) {
				...
				return message.channel.send({ embeds: [notTicket]});
			}
		...
		if (config.commands.close.confirmation) {
			...
			const closeAsk = new MessageEmbed()
			...
			let confirm = await message.channel.send({ embeds: [closeAsk] });
			...
			collector.on('collect', async () => {
				if (channel.id !== message.channel.id) {
					channel.send({ embeds: [closeC] });
				}

				const closeConfirm = new MessageEmbed()
					.setColor(config.colour)
					.setAuthor(message.author.username, message.author.displayAvatarURL())
				...
				confirm.edit({ embeds: [closeConfirm]});
