...
module.exports = {
	...
	async execute(client, [r, u], {config, Ticket, Setting}) {
		...
		const supportRole = channel.guild.roles.cache.get(config.staff_role),
			userRole = channel.guild.roles.cache.get('605527524143005716'),
			elevatedMod = channel.guild.roles.cache.get('734056403777486858'),
			adminRole = channel.guild.roles.cache.get('605529301969928212'),
			ownerRole = channel.guild.roles.cache.get('605524794590625802'),
			// eslint-disable-next-line no-unused-vars
			citizen = channel.guild.roles.cache.get('605527524143005716');
		if (!supportRole) {
			return channel.send(
				new MessageEmbed()
		...
		// This is where we add the server specific overrides. e.g. all 3 mod roles get access to ticket channels 
		// Also we only use the panel
		channel.guild.channels.create(name, {
			...
			{
				id: channel.guild.members.cache.get(u.id), // Ticket Creator
			...
			reason: u.tag + ' requested a new support ticket channel (panel reaction)'
		}).then(async c => {
			Ticket.update({
				channel: c.id
			}, {
			...
			await c.send(` <@&605529301969928212> <@&605525245813719052>, \n ${u} has created a new ticket`);
